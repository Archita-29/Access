...
-- === 20260626180000_contributions.sql ===

-- User-facing RPCs for the address page (username.memact.com).
-- The memact_contributions table already exists from the notebook migration.
...
begin
  select * into target
  from public.memact_contributions c
  where c.id = contribution_id_input and c.user_id = current_user_id;
  ...
  perform public.memact_audit(current_user_id, 'contribution.approve',
    jsonb_build_object('contribution_id', target.id, 'content', target.content));

  return jsonb_build_object('contribution', jsonb_build_object(
    'entry_id', target.id, 'title', target.content, 'status', target.status));
end;
$$;
...
begin
  ...
  update public.memact_contributions set status = 'rejected'
  where id = target.id returning * into target;

  perform public.memact_audit(current_user_id, 'contribution.reject',
...
begin
  ...
  perform public.memact_audit(current_user_id, 'contribution.edit',
    jsonb_build_object('contribution_id', target.id, 'content', target.content));
...
begin
  ...
  perform public.memact_audit(current_user_id, 'contribution.delete',
    jsonb_build_object('contribution_id', target.id));
